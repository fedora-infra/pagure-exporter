...
@click.option(
    ...
    "dkey",
    required=True,
    help="Forgejo API key for accessing the destination namespace",
)
@click.option(
    "-f",
    "--susr",
...
@click.option(
    ...
    "dusr",
    required=True,
    help="Username of the account that owns the Forgejo API key",
)
def main(srce, dest, skey, dkey, susr, dusr):
    """
    Forgejo support for Pagure Exporter
    """
    storeinf(srce, dest, skey, dkey, susr, dusr)
...
def tkts(status, select, ranges, comments, labels, commit, secret, series):
