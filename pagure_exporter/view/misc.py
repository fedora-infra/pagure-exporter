...
def transfer_warning(type, is_available, total_branches):
    if is_available:
        warning(f"Transferring %d requested {type.capitalize()}s" % int(total_branches))
    else:
        warning(f"Transferring %d available {type.capitalize()}s" % int(total_branches))
