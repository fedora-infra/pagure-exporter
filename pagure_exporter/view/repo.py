...
def show_tag_transfer(pushrepo_obj: PushRepo):
    ...
    if source_tag_result[0] and destination_tag_result[0]:
        ...
        section("Initializing namespace assets (tags) transfer...")
        ...
        if transfer_result[0]:
            if standard.tag_transfer_index == standard.tag_transfer_quantity:
                success("Namespace assets (tags) transfer succeeded!")
                sys.exit(0)
            elif 0 < standard.tag_transfer_index < standard.tag_transfer_quantity:
                warning("Namespace assets (tags) transfer partially completed!")
                sys.exit(2)
            else:
                failure("Namespace assets (tags) transfer failed!")
                sys.exit(1)
        else:
            failure("Namespace assets (tags) transfer failed!")
            general("Exception occurred: %s" % str(transfer_result[1]))
    ...
    try:
        ...
        if source_clone_result[0]:
            ...
            if destination_clone_result[0]:
                ...
                show_branch_transfer(pushrepo_obj=pushrepo_obj)
                show_tag_transfer(pushrepo_obj=pushrepo_obj)
            else:  # pragma: no cover
