...
def show_repo():
    try:
        ...
        if source_clone_result[0]:
            ...
            general("Directory: %s" % str(standard.path_source_clone))
            ...
            if destination_clone_result[0]:
                ...
                general("Directory: %s" % str(standard.path_destination_clone))
                ...
                if source_branch_result[0] and destination_branch_result[0]:
                    ...
                    for branch in standard.available_source_branches:
                        general("  - (SRCE branch) %s" % str(branch))
                    general(
                        "Available in destination namespace: %d branch(es)"
                        % len(standard.available_destination_branches)
                    )
                    for branch in standard.available_destination_branches:
                    ...
                    general(
                        ...
                        % (int(standard.tansfer_index), int(standard.tansfer_quantity))
                    )
                    general("Time taken: %s second(s)" % str(transfer_result[1]))
                    if transfer_result[0]:
                        if standard.tansfer_index == standard.tansfer_quantity:
                            success("Namespace assets transfer succeeded!")
                            sys.exit(0)
                        elif 0 < standard.tansfer_index < standard.tansfer_quantity:
