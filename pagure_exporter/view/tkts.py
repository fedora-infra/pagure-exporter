...
def show_tickets():
    ...
    if not standard.ticket_group:
        ...
        if ticket_count_result[0] == 200:
            general(
                f"Found {standard.ticket_count} issue ticket(s) across {standard.page_quantity} "
                f"page(s) in {ticket_count_result[2]} second(s)"
            )
            for page_quantity_index in range(standard.page_quantity):
                section(
                    f"Reading issue tickets information (Page {page_quantity_index + 1} of {standard.page_quantity})..."  # noqa: E501
                )
                ...
                if page_result[0] == 200:
                    ...
                    for page in standard.page_result:
                        ...
                        section(
                            ...
                            f"labels #{standard.issue_identity} '{standard.issue_name}' by "
                        ...
                        if ticket_result[0] == 201:
                            ...
                            if standard.move_comments:
                                ...
                                standard.issue_comments = page["comments"]
                                general(
                                    f"Found {len(standard.issue_comments)} entities in 0.00 second(s)"  # noqa: E501
                                )
                                for issue_comment in standard.issue_comments:
            ...
            general(f"{standard.issues_transfered} issue ticket(s) transferred")
            sys.exit(0)
        else:  # pragma: no cover
        ...
        for ticket_group in standard.ticket_group:
            ...
            if ticket_by_id_result[0] == 200:
                if not ticket_by_id_result[1]:
                    ...
                    ticket_result = movetickets_obj.iterate_tickets(standard.issue_detail)
                    section(
                        f"Migrating issue ticket {'with' if standard.move_labels else 'without'} "
                        f"labels #{standard.issue_identity} '{standard.issue_name}' "
                    ...
                    if ticket_result[0] == 201:
                        ...
                        if standard.move_comments:
                            section("Reading comment information...")
                            standard.issue_comments = standard.issue_detail["comments"]
                            general(
                                f"Found {len(standard.issue_comments)} entities in 0.00 second(s)"
                            )
                            for issue_comment in standard.issue_comments:
                                comment_result = movetickets_obj.iterate_comments(issue_comment)
                                section(
                                    f"Transferring comment (Entity {standard.comment_quantity} of {len(standard.issue_comments)})..."  # noqa: E501
                                )
                                if comment_result[0] == 201:
                                    general(
                                        f"Transferred to {comment_result[1]} in {comment_result[2]} second(s)"  # noqa: E501
