...
class PushRepo:
    def __init__(self):
        self.source_clone_url = standard.source_clone_url
        self.destination_clone_url = standard.destination_clone_url
        ...
        self.source_args = dict(prefix=standard.temp_src_prefix, dir=standard.temp_dir)
        self.destination_args = dict(prefix=standard.temp_dest_prefix, dir=standard.temp_dir)
        ...
        standard.path_source_clone = self.source_location.name
        standard.path_destination_clone = self.destination_location.name

    def clone_source_repo(self):
        try:
            start_time = time.time()
            Repo.clone_from(url=self.source_clone_url, to_path=self.source_location.name)
            stop_time = time.time()
    ...
    def clone_destination_repo(self):
        try:
            start_time = time.time()
            Repo.clone_from(url=self.destination_clone_url, to_path=self.destination_location.name)
            stop_time = time.time()
            return True, "%.2f" % (stop_time - start_time)
        except Exception as expt:
            return False, str(expt)
    ...
    def get_source_branches(self):
        if os.path.exists(os.path.join(self.source_location.name, ".git")):
            ...
            standard.available_source_branches = list(branch_list)
    ...
    def get_destination_branches(self):
        if os.path.exists(os.path.join(self.destination_location.name, ".git")):
            ...
            standard.available_destination_branches = list(branch_list)
            return True, branch_list
        else:
    ...
    def transfer_repo(self):
        if os.path.exists(os.path.join(self.source_location.name, ".git")):
            start_time = time.time()
            repo_obj = Repo(path=self.source_location.name)
            repo_obj.create_remote(standard.new_remote, url=standard.destination_clone_url)
            if len(standard.branches_to_copy) == 0:
                standard.tansfer_quantity = len(standard.available_source_branches)
                transfer_warning(False, standard.tansfer_quantity)
                for brdx in standard.available_source_branches:
                    repo_obj.git.checkout("%s" % brdx)
                    repo_obj.git.push(standard.new_remote, "--set-upstream", brdx, "--force")
                    transfer_progress(
                        brdx,
                        standard.available_source_branches.index(brdx) + 1,
                        len(standard.available_source_branches),
                        True,
                    )
                    standard.tansfer_index += 1
            else:
                standard.tansfer_quantity = len(standard.branches_to_copy)
                transfer_warning(True, standard.tansfer_quantity)
                for brdx in standard.branches_to_copy:
                    if brdx in standard.available_source_branches:
                        repo_obj.git.checkout("%s" % brdx)
                        repo_obj.git.push(standard.new_remote, "--set-upstream", brdx, "--force")
                        transfer_progress(
                            brdx,
