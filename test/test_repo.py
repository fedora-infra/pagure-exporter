...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            f"gitlab --srce {envr['TEST_SRCE']} --dest {envr['TEST_DEST']} --skey {envr['TEST_PKEY']} --dkey {envr['TEST_GKEY']} --susr {envr['TEST_FUSR']} --dusr {envr['TEST_TUSR']} repo",  # noqa: E501
            0,
            [
            ...
            id="GitLab: Migrating repository contents with specifying zero valid branch names",
        ),
        pytest.param(
            f"gitlab --srce {envr['TEST_SRCE']} --dest {envr['TEST_DEST']} --skey {envr['TEST_PKEY']} --dkey {envr['TEST_GKEY']} --susr {envr['TEST_FUSR']} --dusr {envr['TEST_TUSR']} repo --brcs test-aaaa,test-bbbb,test-cccc,test-dddd",  # noqa: E501
            0,
            [
                "[ BUSY ] Attempting source namespace assets clone...",
                "[ PASS ] Source namespace assets clone succeeded!",
            ...
            id="GitLab: Migrating repository contents with specifying four valid branch names",
        ),
        pytest.param(
            f"gitlab --srce {envr['TEST_SRCE']} --dest {envr['TEST_DEST']} --skey {envr['TEST_PKEY']} --dkey {envr['TEST_GKEY']} --susr {envr['TEST_FUSR']} --dusr {envr['TEST_TUSR']} repo --brcs test-aaaa,test-bbbb,test-cxxc,test-dxxd",  # noqa: E501
            2,
            [
                "[ BUSY ] Attempting source namespace assets clone...",
                "[ PASS ] Source namespace assets clone succeeded!",
            ...
            id="GitLab: Migrating repository contents with specifying two valid branch names and two invalid branch names",  # noqa: E501
        ),
        pytest.param(
            f"gitlab --srce {envr['TEST_SRCE']} --dest {envr['TEST_DEST']} --skey {envr['TEST_PKEY']} --dkey {envr['TEST_GKEY']} --susr {envr['TEST_FUSR']} --dusr {envr['TEST_TUSR']} repo --brcs test-axxa,test-bxxb,test-cxxc,test-dxxd",  # noqa: E501
            1,
            [
                "[ BUSY ] Attempting source namespace assets clone...",
                "[ PASS ] Source namespace assets clone succeeded!",
            ...
            id="GitLab: Migrating repository contents with specifying four invalid branch names",
        ),
        pytest.param(
            f"gitlab --srce {envr['TEST_SRCE']} --dest {envr['TEST_DEST']} --skey {envr['TEST_PKEY']} --dkey {envr['TEST_GKEY']} --susr {envr['TEST_FUSR']} --dusr {envr['TEST_TUSR']} repo",  # noqa: E501
            0,
            ...
            id="GitLab: Checking the correctness of metadata censorship for source namespace",
        ),
        pytest.param(
            f"gitlab --srce {envr['TEST_SRCE']} --dest {envr['TEST_DEST']} --skey {envr['TEST_PKEY']} --dkey {envr['TEST_GKEY']} --susr {envr['TEST_FUSR']} --dusr {envr['TEST_TUSR']} repo",  # noqa: E501
            0,
            ...
            id="GitLab: Checking the correctness of metadata censorship for destination namespace",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            id="GitLab: Cloning repository contents of the source namespace into an inoperable location",
        ),
    ],
)
